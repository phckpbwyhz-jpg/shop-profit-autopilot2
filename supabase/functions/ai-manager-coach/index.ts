...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const [{data:settings,error:se},{data:daily,error:de},{data:prior,error:pe}]=await Promise.all([
      admin.from("store_month_settings").select("selling_days_total,sales_goal,labor_goal_pct,parts_goal_pct").eq("store_id",store.id).eq("year",year).eq("month",month).maybeSingle(),
    ...
    const total=safe(settings?.selling_days_total),completed=Math.min(safe(daily?.selling_day_number),total),sales=safe(daily?.mtd_sales),goal=safe(settings?.sales_goal),projected=completed>0?sales/completed*total:0,remainingDays=Math.max(0,total-completed),remainingSales=Math.max(0,goal-sales),needed=remainingDays>0?remainingSales/remainingDays:0,labor=sales>0?safe(daily?.labor_cost)/sales*100:0,parts=sales>0?safe(daily?.parts_cost)/sales*100:0,aro=safe(daily?.car_count_mtd)>0?sales/safe(daily?.car_count_mtd):0,projectedGoal=goal>0?projected/goal*100:0,projectedYoY=prior&&safe(prior.sales)>0?(projected-safe(prior.sales))/safe(prior.sales)*100:null;
    const snapshot={store:{name:store.name,code:store.store_code},period:{year,month,selling_days_completed:completed,selling_days_total:total},sales:{goal,mtd:sales,projected_month_end:projected,projected_goal_pct:projectedGoal,remaining_sales:remainingSales,needed_per_remaining_day:needed},costs:{labor_pct:labor,labor_goal_pct:settings?.labor_goal_pct??null,parts_pct:parts,parts_goal_pct:settings?.parts_goal_pct??null},operations:{car_count_mtd:safe(daily?.car_count_mtd),aro},history:{last_year_final_sales:prior?.sales??null,projected_vs_last_year_pct:projectedYoY}};
    const instructions="You are Shop Profit Autopilot's AI Manager, an automotive shop operations coach. Treat the supplied VERIFIED SHOP SNAPSHOT as the only source of quantitative truth. Never recalculate or contradict its numbers. Do not claim knowledge of open repair orders, future appointments, workload, technician availability, customer intent, or company policy unless explicitly supplied. When discussing labor reductions, use conditional language such as 'if workload allows' and never assume sales impact is zero. Give practical, concise management actions. Separate observed facts from suggested actions. If a question cannot be answered from the snapshot, say what information is missing and what the manager should check.";
  ...
  }catch(error){console.error(error);return json({error:error instanceof Error?error.message:"Unknown error"},500);}
});
