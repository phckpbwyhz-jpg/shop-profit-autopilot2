...
type Form = { name: string; trackingType: KpiTrackingType; goal: string; direction: Direction; required: boolean };
...
export default function KpiBuilderScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data: assignment, error: assignmentError } = await db().from('user_assignments').select('organization_id,district_id,role').eq('user_id', auth.user.id).eq('active', true).order('created_at', { ascending: true }).limit(1).single();
      if (assignmentError) throw assignmentError;
      if (!isAppRole(assignment.role)) throw new Error('Unsupported role.');
      setRole(assignment.role); setOrganizationId(assignment.organization_id); setDistrictId(assignment.district_id);
      ...
      if (storeError) throw storeError; if (definitionError) throw definitionError;
      setStores((storeRows ?? []) as Store[]);
      const defs = (definitionRows ?? []) as Definition[]; setDefinitions(defs);
      if (defs.length) {
        ...
        if (error) throw error; setAssignments((assignmentRows ?? []) as StoreAssignment[]);
      } else setAssignments([]);
    } catch (error) { Alert.alert('KPI Builder', message(error)); }
  ...
  void editingAssignments;
  function toggleStore(storeId: string) { setSelectedStores((current) => current.includes(storeId) ? current.filter((id) => id !== storeId) : [...current, storeId]); }
  function beginEdit(definition: Definition) {
    setEditingId(definition.id);
    setForm({ name: definition.name, trackingType: definition.tracking_type, goal: definition.default_monthly_goal?.toString() ?? '', direction: definition.goal_direction, required: definition.required });
    setSelectedStores(assignments.filter((item) => item.kpi_id === definition.id && item.active).map((item) => item.store_id));
  }
  function reset() { setEditingId(null); setForm(emptyForm); setSelectedStores([]); }
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const user = (await db().auth.getUser()).data.user; if (!user) throw new Error('Sign in is required.');
      const definitionValues = { organization_id: organizationId, district_id: districtId, name: form.name.trim(), tracking_type: form.trackingType, default_monthly_goal: goal, required: form.required, active: true, goal_direction: form.direction };
      let kpiId = editingId;
      if (editingId) { const { error } = await db().from('kpi_definitions').update(definitionValues).eq('id', editingId); if (error) throw error; }
      else { const { data, error } = await db().from('kpi_definitions').insert({ ...definitionValues, created_by: user.id }).select('id').single(); if (error) throw error; kpiId = data.id; }
      if (!kpiId) throw new Error('KPI could not be created.');
      const existing = assignments.filter((item) => item.kpi_id === kpiId);
      await Promise.all(stores.map(async (store) => {
        const row = existing.find((item) => item.store_id === store.id); const active = selectedStores.includes(store.id);
        if (row) { const { error } = await db().from('kpi_store_assignments').update({ active, monthly_goal: goal }).eq('kpi_id', kpiId).eq('store_id', store.id); if (error) throw error; }
        else if (active) { const { error } = await db().from('kpi_store_assignments').insert({ kpi_id: kpiId, store_id: store.id, monthly_goal: goal, active: true }); if (error) throw error; }
      }));
      reset(); await load(); Alert.alert('KPI saved', 'This KPI is now assigned to the selected stores and will flow into their KPI data.');
    } catch (error) { Alert.alert('Could not save KPI', message(error)); }
    finally { setSaving(false); }
  }

  async function toggleActive(definition: Definition) {
    try { const { error } = await db().from('kpi_definitions').update({ active: !definition.active }).eq('id', definition.id); if (error) throw error; await load(); }
  ...
  return <SafeAreaView style={styles.page}>
    <View style={styles.header}><Pressable onPress={() => router.back()}><Ionicons name="chevron-back" size={28} /></Pressable><View style={styles.flex}><Text style={styles.eyebrow}>DISTRICT TOOLS</Text><Text style={styles.headerTitle}>Custom KPI Builder</Text></View><Pressable style={styles.peopleButton} onPress={() => router.push('/users')}><Ionicons name="people-outline" size={21} color="#1769e0" /><Text style={styles.peopleText}>Users</Text></Pressable></View>
